...
Settings.embed_model = NVIDIAEmbedding(model="nvidia/nv-embedqa-e5-v5", api_key=NVIDIA_EMB,truncate="END")


Settings.llm = NVIDIA(model="meta/llama-3.1-8b-instruct", api_key=NVIDIA_MOD)
...
all_tools = [sum_comp_pol_tool, vec_comp_pol_tool, sum_install_tool, vec_install_tool,
             itsup_summ_tool, itsup_summ_vec_tool, knw_base_sum_tool, knw_base_vec_tool,
             sapmle_con_sum_tool, sample_con_vec_tool, test_req_sum_tool, test_req_vec_tool,
...
agent = ReActAgent.from_tools(
    tools=all_tools,
    llm=Settings.llm,
    system_prompt=""" You are an IT support agent and helpful assistant that can answer questions about company IT policies, software installation guides, IT support categories related to the company, company knowledge base articles, learn from past customer sample conversations to answer similar or new csutomer conversations, and also be able to help customers troubleshoot their issues learning from past successful troubleshooting experience.""",
    obj_retriever=obj_retriever,
    max_iterations=60
)
# attach memory so multi‐turn works
agent.memory = Memory.from_defaults(session_id="streamlit_session", token_limit=30000)
